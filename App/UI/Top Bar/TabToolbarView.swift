...
struct TabToolbarItemView: View {
	...
	var body: some View {
		...
		return HStack(spacing: -2) {
			...
			.alert(String.localize("CLOSE_TAB"), isPresented: $showCloseConfirmation) {
				Button(String.localize("CLOSE"), role: .destructive) { removeTerminal() }
				Button(String.localize("CANCEL"), role: .cancel) { }
			} message: {
				Text(String.localize("CLOSE_TAB_CONFIRM_MESSAGE"))
			}
	}
}
